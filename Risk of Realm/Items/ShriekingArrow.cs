...
namespace RiskOfRealm.Items
{
	class ShriekingArrow : ItemBase<ShriekingArrow>
	{
		...
		public override string ItemFullDescription => $"When you initiate your sprint, release up to <style=cIsDamage>{storedArrows}</style> <style=cStack>({storedArrowsPerStack})</style> <style=cIsDamage>homing specter arrows</style> that deal <style=cIsDamage>{arrowDamage * 100}% base damage</style> into nearby enemies, with the specters recharging at a rate of <style=cIsUtility>{arrowRechargeRate} per second</style>.";
		...
		public float arrowRechargeRate;
		...
		protected override void CreateConfig(ConfigFile config)
		{
			...
			arrowRechargeRate = config.Bind("Item: " + ItemName, "Arrow Recharge Rate", 1f).Value;
		}
	}
}